...
function activate(context) {
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('egovframeCodeView', // 여기의 ID는 package.json에서 정의한 view ID와 일치해야 합니다.
...
<body>
  <div class="container">
    ...
    <div class="buttons">
      ...
      <button id="uploadTemplates">Generate Code via Your VM template</button>
    ...
    async generateCode(ddl) {
        ...
        try {
            ...
            const filesToGenerate = {
                [`${tableName}_Mapper.xml`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-mapper-template.vm'), templateContext),
                [`${tableName}List.jsp`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-jsp-list.vm'), templateContext),
                [`${tableName}Register.jsp`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-jsp-register.vm'), templateContext),
                [`${tableName}List.html`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-thymeleaf-list.vm'), templateContext),
                [`${tableName}Register.html`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-thymeleaf-register.vm'), templateContext),
                [`${tableName}Controller.java`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-controller-template.vm'), templateContext),
                [`${tableName}Service.java`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-service-template.vm'), templateContext),
                [`${tableName}DefaultVO.java`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-default-vo-template.vm'), templateContext),
                [`${tableName}VO.java`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-vo-template.vm'), templateContext),
                [`${tableName}ServiceImpl.java`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-service-impl-template.vm'), templateContext),
                [`${tableName}Mapper.java`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-mapper-interface-template.vm'), templateContext),
                [`${tableName}DAO.java`]: await renderTemplate(path.join(this.context.extensionUri.fsPath, 'templates', 'sample-dao-template.vm'), templateContext)
            };
...
function deactivate() { }
//# sourceMappingURL=generateCodeContainer%20copy.js.map