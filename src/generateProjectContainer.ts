...
class TemplateTreeDataProvider
  ...
  private _onDidChangeTreeData: vscode.EventEmitter<
    TemplateTreeItem | undefined | void
  > = new vscode.EventEmitter<TemplateTreeItem | undefined | void>();
  ...
  getChildren(element?: TemplateTreeItem): Thenable<TemplateTreeItem[]> {
    if (!element) {
      ...
      const group = this.templates.find(
        (group) => group.groupName === element.label
      );
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let generateProject = vscode.commands.registerCommand(
      ...
      zipFile.on("entry", (entry) => {
        if (/\/$/.test(entry.fileName)) {
          ...
          const item: SourceItem = {
            fileName:
              lastIndex < 0 ? entryName : entryName.substring(lastIndex + 1),
